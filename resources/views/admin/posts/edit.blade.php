                ...
                <div class="p-6" style="color: var(--color-text-heading)">
                    ...
                    <form action="{{ route('admin.posts.update', $post) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $post->title) }}" class="mt-1 block w-full rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" style="border-color: var(--color-border)" required>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="slug" id="slug" value="{{ old('slug', $post->slug) }}" class="mt-1 block w-full rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" style="border-color: var(--color-border)" required>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="excerpt" id="excerpt" rows="3" class="mt-1 block w-full rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" style="border-color: var(--color-border)">{{ old('excerpt', $post->excerpt) }}</textarea>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="featured_image" id="featured_image" value="{{ old('featured_image', $post->featured_image) }}" class="mt-1 block w-full rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" style="border-color: var(--color-border)">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="visibility" id="visibility" class="mt-1 block w-full rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" style="border-color: var(--color-border)">
                                ...
                                <option value="unlisted" {{ old('visibility', $post->visibility) == 'unlisted' ? 'selected' : '' }}>{{ __('Unlisted') }}</option>
                            </select>
                            @error('visibility') <p class="mt-1 text-sm" style="color: var(--color-error)">{{ $message }}</p> @enderror
                        </div>

                        <div class="mb-4">
                            <label for="content_format" class="block text-sm font-medium" style="color: var(--color-text-body)">{{ __('Content Format') }}</label>
                            <input type="text" name="content_format" id="content_format" value="{{ old('content_format', $post->content_format) }}" class="mt-1 block w-full rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" style="border-color: var(--color-border)">
                            @error('content_format') <p class="mt-1 text-sm" style="color: var(--color-error)">{{ $message }}</p> @enderror
                        </div>

                        <div class="mb-4">
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $post->is_featured) ? 'checked' : '' }} class="rounded shadow-sm focus:ring-indigo-500" style="border-color: var(--color-border); color: var(--color-primary-600)">
