...
<div x-data="{ open: false, selected: '{{ $selected }}' }" class="relative" {{ $attributes }}>
    <button
        @click="open = !open"
        type="button"
    ...
    >
        ...
        <span class="flex-1 text-left" x-text="selected ? options.find(o => o.value === selected)?.label || '{{ $placeholder }}' : '{{ $placeholder }}'"></span>
    ...
    <div x-show="open" @click.away="open = false" class="absolute z-10 mt-1 w-full rounded-lg shadow-lg py-1" style="background-color: var(--color-surface-card); border: 1px solid var(--color-border);">
        @foreach ($options as $option)
            <button
                ...
                :class="selected === '{{ $option['value'] }}' ? 'font-medium' : ''"
                :style="selected === '{{ $option['value'] }}' ? 'color: var(--color-primary-600); background-color: var(--color-primary-50);' : 'color: var(--color-text-body);'"
                onmouseover="this.style.backgroundColor='var(--color-surface-elevated)'"
                onmouseout="this.style.backgroundColor=selected === '{{ $option['value'] }}' ? 'var(--color-primary-50)' : 'transparent'"
            >{{ $option['label'] }}</button>
        @endforeach
    </div>
